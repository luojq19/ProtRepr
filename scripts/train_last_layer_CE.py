...
def main():
    ...
    config.train.freeze_encoder = not args.unfreeze_encoder if args.unfreeze_encoder is not None or not hasattr(config.train, 'freeze_encoder') else config.train.freeze_encoder
    ...
    logger = commons.get_logger('train_last_layer_CE', log_dir)
    writer = SummaryWriter(log_dir)
    logger.info(args)
    logger.info(config)
    
    # Load dataset
    if hasattr(config.data, 'valid_data_file'):
    ...
    model_config = commons.load_config(os.path.join(config.pretrained_model_dir, 'config.yml'))
    model = globals()[model_config.model.model_type](model_config.model)
    ckpt = torch.load(os.path.join(config.pretrained_model_dir, 'checkpoints', 'best_checkpoints.pt'), map_location='cpu')
    model.load_state_dict(ckpt)
    model.to(args.device)
    logger.info(model)
    logger.info(f'Pretrained model loaded from {os.path.join(config.pretrained_model_dir, "checkpoints", "best_checkpoints.pt")}')
    if config.train.freeze_encoder:
        model.freeze_encoder()
        logger.info('Encoder frozen')
    if config.train.load_pretrained_means:
        learned_means = torch.load(os.path.join(config.pretrained_model_dir, 'checkpoints', 'means.pt'), map_location=args.device)
        with torch.no_grad():
            model.layers[-1].weight = torch.nn.Parameter(learned_means)
            model.layers[-1].bias.zero_()
        logger.info(f'Loaded pretrained means as the weight of the last layer.')
    ...
    ec2occurance, label_list = get_ec2occurance(config.data.train_data_file if not hasattr(config.data, 'original_train_data_file') else config.data.original_train_data_file, config.data.label_file, label_name=config.data.label_name, label_level=4)
    logger.info(f'Multi-level evaluation on training set:')
    multi_level_evaluate(model, train_loader, args.device, logger, ec2occurance, label_list, log_dir, tag='train', label_name=config.data.label_name)
